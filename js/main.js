...
function showtask() {
  ...
  taskObj.forEach((item, index) => {
    if (item.completeStatus == true) {
      ...
      btnComplete = `<td><button type="button" class="text-success" id=${index}><i class="fa fa-check-square-o"></i>UnComplete</button></td>`;
    } else {
      taskCompleteValue = `<td>${item.task_name}</td>`;
      btnComplete = `<td><button type="button" class="text-success" id=${index}><i class="fa fa-check-square-o"></i>Complete</button></td>`;
    }
    html += `<tr>
                    <th scope="row">${index + 1}</th>
                    ${taskCompleteValue}
                    ${btnComplete}
                    <td><button type="button" onclick="edittask(${index})" class="text-warning"><i class="fa fa-edit"></i>Edit</button></td>
